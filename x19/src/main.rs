...
const APP_ID: u8 = 19;
const SUITE_NAME: &str = "Argon2id-Grain-128AEADv2-HMAC-SHA256-32768-records";
const PAYLOAD_ALGORITHM: &str = "Grain-128AEADv2";
...
const INNER_TAG_SIZE: usize = 8;
const OUTER_TAG_SIZE: usize = 32;
const TAG_SIZE: usize = INNER_TAG_SIZE + OUTER_TAG_SIZE;
...
fn derive_key(password: &str, salt: &[u8; SALT_SIZE]) -> AppResult<Vec<u8>> {
    ...
    let params = if cfg!(test) {
        argon2::Params::new(8, 1, 1, None).map_err(|error| AppError::new(error.to_string()))?
    } else {
...
fn seal_record(
    ...
    use grain_128aeadv2::{Grain128, aead::KeyInit};
    let (grain_key, mac_key) = key.split_at(16);
    let grain_key: &[u8; 16] = grain_key.try_into().map_err(|_| integrity_error())?;
    let cipher = Grain128::new(&(*grain_key).into());
    let (mut inner_ciphertext, inner_tag) = cipher.encrypt_aead(&(*nonce).into(), aad, message);
    inner_ciphertext.extend_from_slice(&inner_tag);
    let outer_tag = compute_outer_tag(mac_key, nonce, aad, &inner_ciphertext)?;
    inner_ciphertext.extend_from_slice(&outer_tag);
    Ok(inner_ciphertext)
}
...
fn open_record(
    ...
    use grain_128aeadv2::{
        Grain128,
        aead::{KeyInit, array::Array},
    };
    if ciphertext.len() < TAG_SIZE {
        return Err(integrity_error());
    }
    let (grain_key, mac_key) = key.split_at(16);
    let (inner_ciphertext, outer_tag) = ciphertext.split_at(ciphertext.len() - OUTER_TAG_SIZE);
    verify_outer_tag(mac_key, nonce, aad, inner_ciphertext, outer_tag)?;
    let grain_key: &[u8; 16] = grain_key.try_into().map_err(|_| integrity_error())?;
    let cipher = Grain128::new(&(*grain_key).into());
    let (body, inner_tag) = inner_ciphertext.split_at(inner_ciphertext.len() - INNER_TAG_SIZE);
    let inner_tag = Array::try_from(inner_tag).map_err(|_| integrity_error())?;
    cipher
        .decrypt_aead(&(*nonce).into(), aad, body, &inner_tag)
        .map_err(|_| integrity_error())
}

fn compute_outer_tag(
    mac_key: &[u8],
    nonce: &[u8; NONCE_SIZE],
    aad: &[u8],
    inner_ciphertext: &[u8],
) -> AppResult<[u8; OUTER_TAG_SIZE]> {
    use hmac::{Hmac, KeyInit, Mac};
    let mut mac = Hmac::<sha2::Sha256>::new_from_slice(mac_key).map_err(|_| integrity_error())?;
    mac.update(aad);
    mac.update(nonce);
    mac.update(inner_ciphertext);
...
fn verify_outer_tag(
    ...
    mac.update(nonce);
    mac.update(inner_ciphertext);
    mac.verify_slice(outer_tag).map_err(|_| integrity_error())
}
...
mod tests {
    ...
    #[test]
    fn outer_hmac_rejects_inner_and_outer_tampering() {
        let key = [0_u8; KEY_MATERIAL_SIZE];
        let nonce = [0_u8; NONCE_SIZE];
        let aad = b"record metadata";
        let mut inner_tampered = seal_record(&key, &nonce, b"payload", aad).unwrap();
        inner_tampered[0] ^= 1;
        assert!(open_record(&key, &nonce, &inner_tampered, aad).is_err());

        let mut outer_tampered = seal_record(&key, &nonce, b"payload", aad).unwrap();
        let last = outer_tampered.len() - 1;
        outer_tampered[last] ^= 1;
        assert!(open_record(&key, &nonce, &outer_tampered, aad).is_err());
    }
