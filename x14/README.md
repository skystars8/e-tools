...
`x14` encrypts or decrypts one regular file with a human-supplied passphrase.
...
```powershell
cargo build --release -p x14
.\target\release\x14.exe E "input.bin" "input.bin.age"
.\target\release\x14.exe D "input.bin.age" "restored.bin"
```
...
- Temporary files are removed on ordinary failure. A crash can leave a
  dot-prefixed `.x14-*.tmp` file beside the destination; during decryption it
  may contain
