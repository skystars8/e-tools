...
const APP_ID: u8 = 14;
const SUITE_NAME: &str = "scrypt-Strumok-HMAC-SHA384-32768-records";
...
const KEY_MATERIAL_SIZE: usize = 80;
...
fn derive_key(password: &str, salt: &[u8; SALT_SIZE]) -> AppResult<Vec<u8>> {
    ...
    // `Params` restricts its PHC output-length hint to at most 64 bytes. The
    // low-level `scrypt` call below derives the full KEY_MATERIAL_SIZE buffer.
    let params =
        scrypt::Params::new(log_n, 8, 1, 64).map_err(|error| AppError::new(error.to_string()))?;
...
fn record_mac(
    ...
    nonce: &[u8],
    ciphertext: &[u8],
) -> AppResult<hmac::Hmac<sha2::Sha384>> {
    use hmac::{KeyInit, Mac};
    let mut mac = <hmac::Hmac<sha2::Sha384> as KeyInit>::new_from_slice(key)
...
fn seal_record(
    ...
    dstu8845::Dstu8845_256::new_from_slices(cipher_key, nonce)
        .map_err(|_| integrity_error())?
