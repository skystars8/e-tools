...
const APP_ID: u8 = 5;
const SUITE_NAME: &str = "PBKDF2-SHA384-XSalsa20-Poly1305-secretbox-32768-records";
...
const TAG_SIZE: usize = 16 + HEADER_SIZE + 13;
...
fn seal_record(
    ...
    libsodium_rs::ensure_init().map_err(|_| integrity_error())?;
    let key = crypto_secretbox::Key::from_bytes(key).map_err(|_| integrity_error())?;
    let nonce = crypto_secretbox::Nonce::from_bytes(*nonce);
    let mut bound = Vec::with_capacity(aad.len() + message.len());
    bound.extend_from_slice(aad);
    bound.extend_from_slice(message);
    Ok(crypto_secretbox::seal(&bound, &nonce, &key))
}
...
fn open_record(
    ...
    let nonce = crypto_secretbox::Nonce::from_bytes(*nonce);
    let plaintext =
        crypto_secretbox::open(ciphertext, &nonce, &key).map_err(|_| integrity_error())?;
    if plaintext.len() < aad.len() || plaintext[..aad.len()] != *aad {
        return Err(integrity_error());
    }
    Ok(plaintext[aad.len()..].to_vec())
}
