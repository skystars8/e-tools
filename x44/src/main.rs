...
const APP_ID: u8 = 44;
const PAYLOAD_ALGORITHM: &str = "Threefish-256";
const SUITE_NAME: &str = "PBKDF2-SHA256 / Threefish-256 / HMAC-SHA256-EtM";
...
fn record_tweak(prefix: &[u8; PREFIX_SIZE], record: u64) -> [u8; 16] {
    let mut tweak = *prefix;
    for (slot, value) in tweak[8..].iter_mut().zip(record.to_be_bytes()) {
        *slot ^= value;
    }
    tweak
}

fn counter_block(prefix: &[u8; PREFIX_SIZE], record: u64, block_index: u64) -> [u8; 32] {
    let mut block = [0_u8; 32];
    block[..16].copy_from_slice(prefix);
    block[16..].copy_from_slice(prefix);
    for (slot, value) in block[8..16].iter_mut().zip(record.to_be_bytes()) {
        *slot ^= value;
    }
    for (slot, value) in block[24..].iter_mut().zip(block_index.to_be_bytes()) {
        *slot ^= value;
    }
    block
}
...
fn transform_payload(
    ...
    use threefish::cipher::{Array, BlockCipherEncrypt};
    let key_array: [u8; 32] = key.try_into().map_err(|_| integrity_error())?;
    let cipher = threefish::Threefish256::new_with_tweak(&key_array, &record_tweak(prefix, record));
