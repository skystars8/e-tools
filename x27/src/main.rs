...
const APP_ID: u8 = 27;
const SUITE_NAME: &str = "PBKDF2-SHA224-SPECK128-256-EAX-262144-records";
const PAYLOAD_ALGORITHM: &str = "SPECK128-256";
...
fn seal_record(
    ...
    use cryptography::{Eax, Speck128_256};
    let key: &[u8; 32] = key.try_into().map_err(|_| integrity_error())?;
    let eax = Eax::new(Speck128_256::new(key));
...
fn open_record(
    ...
    if !Eax::new(Speck128_256::new(key)).decrypt(nonce, aad, &mut plaintext, tag) {
        plaintext.fill(0);
        return Err(integrity_error());
    }
    Ok(plaintext)
