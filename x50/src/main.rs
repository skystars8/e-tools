...
const APP_ID: u8 = 50;
const PAYLOAD_ALGORITHM: &str = "RC4-drop3072";
const SUITE_NAME: &str = "PBKDF2-SHA256 / RC4-drop3072 / HMAC-SHA256-EtM";
...
fn record_key(key: &[u8], prefix: &[u8; PREFIX_SIZE], record: u64) -> [u8; 32] {
    let mut derivation =
        <RecordMac as Mac>::new_from_slice(key).expect("HMAC accepts any key length");
    derivation.update(b"x50 rc4 per-record key");
    derivation.update(prefix);
    derivation.update(&record.to_be_bytes());
...
fn transform_payload(
    ...
    let mut key = record_key(key, prefix, record);
    let mut cipher = rc4::Rc4::new(&key.into());
    let mut discarded = [0_u8; 3072];
