...
x50 is a standalone Rust CLI for password-based file encryption and
...
~~~powershell
cargo build --release -p x50
.\target\release\x50.exe E "input.bin" "encrypted.age"
.\target\release\x50.exe D "encrypted.age" "restored.bin"
~~~
...
- Ordinary failures remove the temporary file. A machine crash can leave a
  .x50-*.tmp file; during decryption, that file can contain plaintext.
- Decryption rejects malformed or non-passphrase age files and caps the parsed
  header at 64 KiB.

x50 files are compatible with x1 through x50 and other implementations of the
age v1 passphrase format. Run its tests with cargo test -p x50 --locked.
