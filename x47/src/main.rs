...
const APP_ID: u8 = 47;
const PAYLOAD_ALGORITHM: &str = "Rabbit";
const SUITE_NAME: &str = "PBKDF2-SHA256 / Rabbit / HMAC-SHA256-EtM";
...
fn record_iv(prefix: &[u8; PREFIX_SIZE], record: u64) -> [u8; 8] {
    let mut iv: [u8; 8] = prefix[..8].try_into().expect("prefix slice is fixed");
    for (slot, value) in iv.iter_mut().zip(record.to_be_bytes()) {
        *slot ^= value;
    }
    iv
}
...
fn transform_payload(
    ...
    let key_array: [u8; 16] = key.try_into().map_err(|_| integrity_error())?;
    let iv = record_iv(prefix, record);
    let mut cipher = rabbit::Rabbit::new(&key_array.into(), &iv.into());
