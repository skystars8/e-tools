...
const APP_ID: u8 = 30;
const SUITE_NAME: &str = "PBKDF2-SHA512-IDEA-128-CTR-HMAC-SHA256-65536-records";
const PAYLOAD_ALGORITHM: &str = "IDEA-128";
...
fn apply_payload_cipher(
    ...
    type Cipher = ctr::Ctr64BE<idea::Idea>;
    let enc_key: &[u8; 16] = enc_key.try_into().map_err(|_| integrity_error())?;
    Cipher::new(&(*enc_key).into(), &(*nonce).into()).apply_keystream(data);
    Ok(())
}
