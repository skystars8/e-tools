...
const APP_ID: u8 = 31;
const SUITE_NAME: &str = "Argon2id-SEED-128-EAX-262144-records";
const PAYLOAD_ALGORITHM: &str = "SEED-128";
...
const KEY_MATERIAL_SIZE: usize = 16;
...
fn seal_record(
    ...
    use cryptography::{Eax, SeedCt};
    let key: &[u8; 16] = key.try_into().map_err(|_| integrity_error())?;
    let eax = Eax::new(SeedCt::new(key));
    let mut ciphertext = message.to_vec();
    let tag = eax.encrypt(nonce, aad, &mut ciphertext);
...
fn open_record(
    ...
    let tag: &[u8; 16] = tag.try_into().map_err(|_| integrity_error())?;
    let mut plaintext = body.to_vec();
    if !Eax::new(SeedCt::new(key)).decrypt(nonce, aad, &mut plaintext, tag) {
        plaintext.fill(0);
        return Err(integrity_error());
    }
    Ok(plaintext)
