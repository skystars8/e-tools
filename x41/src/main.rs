...
const APP_ID: u8 = 41;
const PAYLOAD_ALGORITHM: &str = "Blowfish";
const SUITE_NAME: &str = "PBKDF2-SHA256 / Blowfish / HMAC-SHA256-EtM";
...
fn transform_payload(
    ...
    let cipher =
        blowfish::Blowfish::<byteorder::BE>::new_from_slice(key).map_err(|_| integrity_error())?;
    let mut output = input.to_vec();
    for (block_index, chunk) in output.chunks_mut(8).enumerate() {
        let seed = counter_block(prefix, record, block_index)?;
        let mut stream = Block::<blowfish::Blowfish<byteorder::BE>>::clone_from_slice(&seed);
