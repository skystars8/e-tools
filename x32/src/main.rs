...
const APP_ID: u8 = 32;
const SUITE_NAME: &str = "scrypt-PRESENT-128-CTR-HMAC-SHA256-1048576-records";
const PAYLOAD_ALGORITHM: &str = "PRESENT-128";
...
fn derive_key(password: &str, salt: &[u8; SALT_SIZE]) -> AppResult<Vec<u8>> {
    ...
    let log_n = if cfg!(test) { 4 } else { 14 };
    let params = scrypt::Params::new(log_n, 8, 1, key.len())
        .map_err(|error| AppError::new(error.to_string()))?;
...
fn apply_payload_cipher(
    ...
    let enc_key: &[u8; 16] = enc_key.try_into().map_err(|_| integrity_error())?;
    cryptography::Ctr::new(cryptography::Present128Ct::new(enc_key)).apply_keystream(nonce, data);
    Ok(())
}
