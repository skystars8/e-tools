...
const APP_ID: u8 = 20;
const SUITE_NAME: &str = "scrypt-Romulus-M-65536-records";
const PAYLOAD_ALGORITHM: &str = "Romulus-M";
...
fn seal_record(
    ...
    use aead::{Aead, KeyInit, Payload, array::Array};
    let key = Array::try_from(key).map_err(|_| integrity_error())?;
    let nonce = Array::from(*nonce);
    lib_q_romulus::RomulusM::new(&key)
