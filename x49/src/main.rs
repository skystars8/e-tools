...
const APP_ID: u8 = 49;
const PAYLOAD_ALGORITHM: &str = "Skipjack";
const SUITE_NAME: &str = "PBKDF2-SHA256 / Skipjack / HMAC-SHA256-EtM";
...
const ENC_KEY_SIZE: usize = 10;
const KDF_ROUNDS: u32 = 184000;
...
fn counter_word(prefix: &[u8; PREFIX_SIZE], record: u64, block_index: usize) -> AppResult<u64> {
    let record = u32::try_from(record).map_err(|_| integrity_error())?;
    let block_index = u32::try_from(block_index).map_err(|_| integrity_error())?;
    let mask = u64::from_be_bytes(prefix[..8].try_into().map_err(|_| integrity_error())?);
    Ok(mask ^ (u64::from(record) << 32) ^ u64::from(block_index))
}
...
fn transform_payload(
    ...
    let key_array: [u8; 10] = key.try_into().map_err(|_| integrity_error())?;
    let mut output = input.to_vec();
    for (block_index, chunk) in output.chunks_mut(8).enumerate() {
        let stream = skipjack::skipjack::encrypt_block(
            counter_word(prefix, record, block_index)?,
            key_array,
        )
        .to_be_bytes();
...
mod tests {
    ...
    #[test]
    fn skipjack_nist_known_answer_vector() {
        let key = [0x00, 0x99, 0x88, 0x77, 0x66, 0x55, 0x44, 0x33, 0x22, 0x11];
        assert_eq!(
            skipjack::skipjack::encrypt_block(0x33221100ddccbbaa, key),
            0x2587cae27a12d300
        );
    }
}
