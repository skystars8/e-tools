...
const APP_ID: u8 = 9;
const SUITE_NAME: &str = "PBKDF2-SHA224-Gimli-AEAD-1048576-records";
...
fn seal_record(
    ...
    let tag = gimli_aead::GimliAead::new(key, nonce).encrypt(aad, &mut ciphertext);
...
fn open_record(
    ...
    if !gimli_aead::GimliAead::new(key, nonce).decrypt(aad, &mut plaintext, tag) {
        return Err(integrity_error());
    }
    Ok(plaintext)
}
