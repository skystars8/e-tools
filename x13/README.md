...
`x13` is a password-based file encryptor and decryptor. It reads one regular file and writes the encrypted or recovered bytes to a different path; the input is never changed.
...
```powershell
cargo build --release --locked -p x13
.\target\release\x13.exe E "input.bin" "input.bin.age"
.\target\release\x13.exe D "input.bin.age" "restored.bin"
```

On Unix-like systems, run `./target/release/x13` instead. The CLI accepts exactly three arguments: uppercase `E` or `D`, an input path, and an output path. File data cannot be supplied through standard input or written to standard output.
...
- Temporary output is created beside the requested destination and removed on ordinary failure. A process or machine crash can leave a dot-prefixed `.x13-*.tmp` file; during decryption, that temporary file can contain plaintext.
