...
const APP_ID: u8 = 10;
const SUITE_NAME: &str = "PBKDF2-SHA256-Xoodyak-32768-records";
...
fn seal_record(
    ...
    let mut cipher =
        XoodyakKeyed::new(key, Some(nonce), None, None).map_err(|_| integrity_error())?;
    cipher.absorb(aad);
    cipher
        .aead_encrypt_to_vec(Some(message))
...
fn open_record(
    ...
    cipher.absorb(aad);
    cipher
        .aead_decrypt_to_vec(ciphertext)
        .map_err(|_| integrity_error())
}
