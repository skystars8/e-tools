...
const APP_ID: u8 = 46;
const PAYLOAD_ALGORITHM: &str = "Spritz";
const SUITE_NAME: &str = "PBKDF2-SHA256 / Spritz / HMAC-SHA256-EtM";
...
fn record_nonce(prefix: &[u8; PREFIX_SIZE], record: u64) -> [u8; 16] {
    let mut nonce = *prefix;
    for (slot, value) in nonce[8..].iter_mut().zip(record.to_be_bytes()) {
        *slot ^= value;
    }
    nonce
}
...
fn transform_payload(
    ...
    let nonce = record_nonce(prefix, record);
    let mut cipher = spritz_cipher::SpritzCipherContext::setup_with_IV(key, &nonce);
    let mut output = vec![0_u8; input.len()];
    cipher
        .crypt(input, &mut output)
        .map_err(|_| integrity_error())?;
    Ok(output)
}
