...
const APP_ID: u8 = 34;
const SUITE_NAME: &str = "PBKDF2-SHA256-Trivium-HMAC-SHA256-65536-records";
const PAYLOAD_ALGORITHM: &str = "Trivium";
...
const KEY_MATERIAL_SIZE: usize = 42;
...
fn apply_payload_cipher(
    ...
    let enc_key: &[u8; 10] = enc_key.try_into().map_err(|_| integrity_error())?;
    let input = data.to_vec();
    let output = trivium::Trivium::new(
        enc_key,
        nonce,
        trivium::BitOrder::Msb,
        trivium::PackOrder::Lsb,
    )
    .xor_bytes(&input);
