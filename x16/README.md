...
`x16` encrypts or decrypts one regular file with a human-supplied passphrase.
...
```powershell
cargo build --release -p x16
.\target\release\x16.exe E "input.bin" "input.bin.age"
.\target\release\x16.exe D "input.bin.age" "restored.bin"
```
...
- Temporary files are removed on ordinary failure. A crash can leave a
  dot-prefixed `.x16-*.tmp` file beside the destination; during decryption it
  may contain
