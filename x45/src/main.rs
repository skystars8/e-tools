...
const APP_ID: u8 = 45;
const PAYLOAD_ALGORITHM: &str = "LEA-256";
const SUITE_NAME: &str = "PBKDF2-SHA256 / LEA-256 / HMAC-SHA256-EtM";
...
fn transform_payload(
    ...
    use lea::prelude::{BlockEncrypt, GenericArray, NewBlockCipher};
    let cipher = lea::Lea256::new(GenericArray::from_slice(key));
...
mod tests {
    ...
    #[test]
    fn lea_256_known_answer_vector() {
        use lea::prelude::{BlockEncrypt, GenericArray, NewBlockCipher};
        let key = [
        ...
        lea::Lea256::new(GenericArray::from_slice(&key)).encrypt_block(&mut block);
        assert_eq!(
            block.as_slice(),
            &[
