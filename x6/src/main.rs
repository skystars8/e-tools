...
const APP_ID: u8 = 6;
const SUITE_NAME: &str = "PBKDF2-SHA512-Deoxys-II-256-65536-records";
...
fn seal_record(
    ...
    let cipher = deoxys::DeoxysII256::new_from_slice(key).map_err(|_| integrity_error())?;
    let nonce =
        aead::Nonce::<deoxys::DeoxysII256>::try_from(&nonce[..]).map_err(|_| integrity_error())?;
    cipher
