...
const APP_ID: u8 = 42;
const PAYLOAD_ALGORITHM: &str = "BELT-DWP";
const SUITE_NAME: &str = "PBKDF2-SHA256 / BELT-DWP / HMAC-SHA256-EtM";
...
type BeltDwp128 = belt_dwp::Dwp<belt_dwp::BeltBlock, belt_dwp::aead::consts::U16>;

fn belt_nonce(prefix: &[u8; PREFIX_SIZE], record: u64) -> belt_dwp::aead::Nonce<BeltDwp128> {
    let mut bytes = *prefix;
    for (slot, value) in bytes[8..].iter_mut().zip(record.to_be_bytes()) {
        *slot ^= value;
    }
    belt_dwp::aead::Nonce::<BeltDwp128>::from(bytes)
}
...
fn encrypt_payload(
    ...
    use belt_dwp::aead::{Aead, KeyInit, Payload};
    let key_array: [u8; 32] = key.try_into().map_err(|_| integrity_error())?;
    let cipher = BeltDwp128::new(&key_array.into());
    cipher
        .encrypt(
            &belt_nonce(prefix, record),
            Payload {
                msg: plaintext,
                aad,
            },
