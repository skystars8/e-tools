...
const APP_ID: u8 = 17;
const SUITE_NAME: &str = "PBKDF2-SHA384-Turing-HMAC-SHA512-262144-records";
...
const KEY_MATERIAL_SIZE: usize = 96;
...
fn record_mac(
    ...
    nonce: &[u8],
    ciphertext: &[u8],
) -> AppResult<hmac::Hmac<sha2_mac::Sha512>> {
    use hmac::{KeyInit, Mac};
    let mut mac = <hmac::Hmac<sha2_mac::Sha512> as KeyInit>::new_from_slice(key)
...
fn turing_xor(key: &[u8; 32], nonce: &[u8; 16], input: &[u8]) -> AppResult<Vec<u8>> {
    let mut cipher = turing_cipher::Turing::new();
    cipher
        .turing_key(key, key.len())
        .map_err(|_| integrity_error())?;
    cipher
        .turing_iv(nonce, nonce.len())
        .map_err(|_| integrity_error())?;
    let mut output = Vec::with_capacity(input.len());
    for chunk in input.chunks(340) {
        let mut keystream = [0_u8; 340];
        cipher
            .turing_gen(&mut keystream)
            .map_err(|_| integrity_error())?;
...
fn seal_record(
    ...
    let cipher_key: &[u8; 32] = cipher_key.try_into().map_err(|_| integrity_error())?;
    let mut ciphertext = turing_xor(cipher_key, nonce, message)?;
    let tag = record_mac(mac_key, aad, nonce, &ciphertext)?
        .finalize()
        .into_bytes();
...
fn open_record(
    ...
    let (cipher_key, mac_key) = key.split_at(32);
    record_mac(mac_key, aad, nonce, body)?
        .verify_slice(tag)
        .map_err(|_| integrity_error())?;
    let cipher_key: &[u8; 32] = cipher_key.try_into().map_err(|_| integrity_error())?;
    turing_xor(cipher_key, nonce, body)
}
