...
const APP_ID: u8 = 23;
const SUITE_NAME: &str = "PBKDF2-SHA384-BLAKE3-AEAD-1048576-records";
const PAYLOAD_ALGORITHM: &str = "BLAKE3-AEAD";
...
fn seal_record(
    ...
    let key: &[u8; blake3_aead::KEY_LEN] = key.try_into().map_err(|_| integrity_error())?;
    Ok(blake3_aead::encrypt(key, nonce, aad, message))
}
...
fn open_record(
    ...
    let key: &[u8; blake3_aead::KEY_LEN] = key.try_into().map_err(|_| integrity_error())?;
    blake3_aead::decrypt(key, nonce, aad, ciphertext).map_err(|_| integrity_error())
}
