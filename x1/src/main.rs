...
const APP_ID: u8 = 1;
const SUITE_NAME: &str = "Argon2id-AES-256-GCM-65536-records";
...
fn seal_record(
    ...
    let cipher = aes_gcm::Aes256Gcm::new_from_slice(key).map_err(|_| integrity_error())?;
    let nonce =
        aead::Nonce::<aes_gcm::Aes256Gcm>::try_from(&nonce[..]).map_err(|_| integrity_error())?;
    cipher
