use crate::common::{absorb, branch_x4, merge_x4, pad, tags, with, with_x4, Tag};
use crate::constant::{BLOCK_LENGTH, KEY_LENGTH, S, TAG_LENGTH, U};
use crate::{permutation, Decrypt, Encrypt, Norx};
...
impl Process<Encrypt> {
    ...
    where
        I: Iterator<Item = (&'a [u8; BLOCK_LENGTH], &'a mut [u8; BLOCK_LENGTH])>,
    {
        ...
        for (input, output) in buff.iter_mut().filter_map(|buf| buf.take()).fuse() {
            {
