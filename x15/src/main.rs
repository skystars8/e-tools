...
const APP_ID: u8 = 15;
const SUITE_NAME: &str = "PBKDF2-SHA224-Enocoro-128v2-HMAC-SHA3-256-65536-records";
...
fn record_mac(
    ...
    nonce: &[u8],
    ciphertext: &[u8],
) -> AppResult<hmac::Hmac<sha3::Sha3_256>> {
    use hmac::{KeyInit, Mac};
    let mut mac = <hmac::Hmac<sha3::Sha3_256> as KeyInit>::new_from_slice(key)
...
fn seal_record(
    ...
    enocoro128v2::Enocoro128::new(cipher_key, nonce).apply_keystream(&mut ciphertext);
...
fn open_record(
    ...
    enocoro128v2::Enocoro128::new(cipher_key, nonce).apply_keystream(&mut plaintext);
    Ok(plaintext)
}
