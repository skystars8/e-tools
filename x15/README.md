...
`x15` encrypts or decrypts one regular file with a human-supplied passphrase.
...
```powershell
cargo build --release -p x15
.\target\release\x15.exe E "input.bin" "input.bin.age"
.\target\release\x15.exe D "input.bin.age" "restored.bin"
```
...
- Temporary files are removed on ordinary failure. A crash can leave a
  dot-prefixed `.x15-*.tmp` file beside the destination; during decryption it
  may contain
