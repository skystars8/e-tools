...
- KDF: PBKDF2-SHA256
- Payload algorithm: CAST6-256
- Record protection: CAST6-CTR + HMAC-SHA256 Encrypt-then-MAC
...
Usage: `x28 E|D <input> <output>`.

The header, record metadata, record order, and final-record marker are authenticated. Output is staged in the destination directory and committed without overwriting an existing file. x28 deliberately rejects every sibling format.
