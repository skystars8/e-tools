...
const APP_ID: u8 = 48;
const PAYLOAD_ALGORITHM: &str = "TEA-32";
const SUITE_NAME: &str = "PBKDF2-SHA256 / TEA-32 / HMAC-SHA256-EtM";
...
fn counter_block(
    ...
    let mut block: [u8; 8] = prefix[..8].try_into().map_err(|_| integrity_error())?;
    for (slot, value) in block[..4].iter_mut().zip(record.to_be_bytes()) {
        *slot ^= value;
    }
    for (slot, value) in block[4..].iter_mut().zip(block_index.to_be_bytes()) {
        *slot ^= value;
    }
    Ok(block)
}
...
fn transform_payload(
    ...
    use tea_soft::block_cipher::{BlockCipher, NewBlockCipher, generic_array::GenericArray};
    let cipher = tea_soft::Tea32::new(GenericArray::from_slice(key));
    let mut output = input.to_vec();
    for (block_index, chunk) in output.chunks_mut(8).enumerate() {
        let seed = counter_block(prefix, record, block_index)?;
        let mut stream = GenericArray::clone_from_slice(&seed);
