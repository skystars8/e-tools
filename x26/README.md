...
`x26` encrypts or decrypts one regular file with a human-supplied passphrase.
...
```powershell
cargo build --release -p x26
.\target\release\x26.exe E "input.bin" "input.bin.age"
.\target\release\x26.exe D "input.bin.age" "restored.bin"
```
...
- Temporary files are removed on ordinary failure. A crash can leave a
  dot-prefixed `.x26-*.tmp` file beside the destination; during decryption it
  may contain
