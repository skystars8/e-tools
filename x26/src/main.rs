...
const APP_ID: u8 = 26;
const SUITE_NAME: &str = "scrypt-SIMON128-256-EAX-131072-records";
const PAYLOAD_ALGORITHM: &str = "SIMON128-256";
...
fn seal_record(
    ...
    use cryptography::{Eax, Simon128_256};
    let key: &[u8; 32] = key.try_into().map_err(|_| integrity_error())?;
    let eax = Eax::new(Simon128_256::new(key));
...
fn open_record(
    ...
    if !Eax::new(Simon128_256::new(key)).decrypt(nonce, aad, &mut plaintext, tag) {
        plaintext.fill(0);
        return Err(integrity_error());
    }
    Ok(plaintext)
