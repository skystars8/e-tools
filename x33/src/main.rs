...
const APP_ID: u8 = 33;
const SUITE_NAME: &str = "PBKDF2-SHA224-Magma-256-CTR-HMAC-SHA256-32768-records";
const PAYLOAD_ALGORITHM: &str = "Magma";
...
fn derive_key(password: &str, salt: &[u8; SALT_SIZE]) -> AppResult<Vec<u8>> {
    ...
    let rounds = if cfg!(test) { 1 } else { 230_000 };
    pbkdf2::pbkdf2_hmac::<sha2::Sha224>(password.as_bytes(), salt, rounds, &mut key);
    Ok(key)
}

fn apply_payload_cipher(
    enc_key: &[u8],
    nonce: &[u8; NONCE_SIZE],
    data: &mut [u8],
) -> AppResult<()> {
    let enc_key: &[u8; 32] = enc_key.try_into().map_err(|_| integrity_error())?;
    cryptography::Ctr::new(cryptography::MagmaCt::new(enc_key)).apply_keystream(nonce, data);
    Ok(())
}
...
fn compute_tag(
    ...
    aad: &[u8],
    ciphertext: &[u8],
) -> AppResult<[u8; TAG_SIZE]> {
    use hmac::{Hmac, KeyInit, Mac};
    let mut mac =
        Hmac::<sha2_011::Sha256>::new_from_slice(mac_key).map_err(|_| integrity_error())?;
    mac.update(aad);
    mac.update(nonce);
...
fn verify_tag(
    ...
    mac.update(nonce);
    mac.update(ciphertext);
    mac.verify_slice(tag).map_err(|_| integrity_error())
}
...
fn seal_record(
    ...
    let (enc_key, mac_key) = key.split_at(KEY_MATERIAL_SIZE - 32);
    let mut ciphertext = message.to_vec();
    apply_payload_cipher(enc_key, nonce, &mut ciphertext)?;
    let tag = compute_tag(mac_key, nonce, aad, &ciphertext)?;
...
fn open_record(
    ...
    verify_tag(mac_key, nonce, aad, body, tag)?;
    let mut plaintext = body.to_vec();
    apply_payload_cipher(enc_key, nonce, &mut plaintext)?;
    Ok(plaintext)
}
...
fn make_nonce(_prefix: &[u8; PREFIX_SIZE], counter: u64) -> AppResult<[u8; NONCE_SIZE]> {
    let record = u32::try_from(counter).map_err(|_| integrity_error())?;
...
mod tests {
    ...
    #[test]
    fn adjacent_record_counters_do_not_overlap() {
        const BLOCK_SIZE: usize = 8;
        assert!(CHUNK_SIZE.div_ceil(BLOCK_SIZE) <= u32::MAX as usize);
        let key = [0_u8; 32];
        let prefix = [0_u8; PREFIX_SIZE];
        let mut record_zero = [0_u8; BLOCK_SIZE * 2];
        let mut record_one = [0_u8; BLOCK_SIZE];
        apply_payload_cipher(&key, &make_nonce(&prefix, 0).unwrap(), &mut record_zero).unwrap();
        apply_payload_cipher(&key, &make_nonce(&prefix, 1).unwrap(), &mut record_one).unwrap();
        assert_ne!(&record_zero[BLOCK_SIZE..], &record_one);
        assert!(make_nonce(&prefix, u64::from(u32::MAX) + 1).is_err());
    }
