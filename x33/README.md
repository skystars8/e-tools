...
- KDF: PBKDF2-SHA224
- Payload algorithm: Magma
- Record protection: Magma-CTR + HMAC-SHA256 Encrypt-then-MAC
...
Usage: `x33 E|D <input> <output>`.

The header, record metadata, record order, and final-record marker are authenticated. Output is staged in the destination directory and committed without overwriting an existing file. x33 deliberately rejects every sibling format.
