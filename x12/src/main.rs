...
const APP_ID: u8 = 12;
const SUITE_NAME: &str = "PBKDF2-SHA512-KCipher-2-HMAC-SHA256-262144-records";
...
fn record_mac(
    ...
    nonce: &[u8],
    ciphertext: &[u8],
) -> AppResult<hmac::Hmac<sha2_mac::Sha256>> {
    use hmac::{KeyInit, Mac};
    let mut mac = <hmac::Hmac<sha2_mac::Sha256> as KeyInit>::new_from_slice(key)
...
fn seal_record(
    ...
    kcipher2::KCipher2::new_from_slices(cipher_key, nonce)
        .map_err(|_| integrity_error())?
